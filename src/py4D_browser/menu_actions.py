...
def load_data_EMPAD2(self,):
    ...
    if hasattr(self, "_EMPAD_bkg_odd"):
        self.datacube = _load_EMPAD2_datacube(filepath, self._EMPAD_bkg_even, self._EMPAD_bkg_odd)
...
def load_data_background_EMPAD2(self,):
    filepath = self.show_file_dialog()
    ...
    print("Successfully loaded EMPAD2 background data... Ready to load experimental data.")
...
def _load_EMPAD2_datacube(filepath, background_even=None, background_odd=None):
    ...
    ), "Did you do a non-square scan?"

    data_shape = (int(Nxy), int(Nxy), 128, 128)

    with open(filepath, "rb") as fid:
        datacube = py4DSTEM.DataCube(
            np.fromfile(fid, np.float32).reshape(data_shape)
        )
...
def _process_EMPAD2_datacube(datacube, background_even=None, background_odd=None):
    ...
    for rx,ry in py4DSTEM.tqdmnd(datacube.data.shape[0], datacube.data.shape[1]):
        ...
        if background_even is not None and background_odd is not None:
            if ry % 2:
                # odd frame
                datacube.data[rx,ry] = _FFB * ((analog * (1-gain_bit) + _G1B * (analog - _B2B) * gain_bit + _G2B * digital) - background_odd)
            else:
                # even frame
                datacube.data[rx,ry] = _FFA * ((analog * (1-gain_bit) + _G1A * (analog - _B2A) * gain_bit + _G2A * digital) - background_even)
        else:
            if ry % 2:
                # odd frame
                datacube.data[rx,ry] = analog * (1-gain_bit) + _G1B * (analog - _B2B) * gain_bit + _G2B * digital
            else:
                # even frame
                datacube.data[rx,ry] = analog * (1-gain_bit) + _G1A * (analog - _B2A) * gain_bit + _G2A * digital
